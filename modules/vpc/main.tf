...
resource "aws_vpc" "default" {
  ...
  tags = "${merge(var.default_tags, map( "Name", "${var.name}-vpc" ))}"
}
...
resource "aws_internet_gateway" "igw" {
  ...
  tags = "${merge(var.default_tags, map( "Name", "${var.name}-igw" ))}"
}
...
resource "aws_subnet" "public" {
  ...
  tags = "${merge(var.default_tags, map( "Name", "${var.name}-public-${element(data.aws_availability_zones.available.names, count.index)}"))}"
}
...
resource "aws_subnet" "private" {
  ...
  tags = "${merge(var.default_tags, map( "Name", "${var.name}-private-${element(data.aws_availability_zones.available.names, count.index)}"))}"
}
...
resource "aws_route_table" "public" {
  ...
  tags = "${merge(var.default_tags, map( "Name", "${var.name}-rt-public"))}"
}
...
resource "aws_route_table" "private" {
  ...
  tags = "${merge(var.default_tags, map( "Name", "${var.name}-rt-private-${count.index}"))}"
}
